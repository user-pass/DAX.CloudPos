...
export default class SalesTransSignaturePreEndTransactionTrigger extends Triggers.PreEndTransactionTrigger {
    /**
    ...
    public execute(options: Triggers.IPreEndTransactionTriggerOptions): Promise<ClientEntities.ICancelable> {
        if (options.cart.CartTypeValue !== ProxyEntities.CartType.Shopping && options.cart.CartTypeValue !== ProxyEntities.CartType.CustomerOrder) {
        ...
        return signatureManager.preSignature(options.cart)
