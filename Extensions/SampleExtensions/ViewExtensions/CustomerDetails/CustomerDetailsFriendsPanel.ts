...
export default class CustomerDetailsFriendsPanel extends CustomerDetailsCustomControlBase {
    public readonly friends: ObservableArray<ProxyEntities.Customer>;
    public readonly dataList: DataList<ProxyEntities.Customer>;
    ...
    constructor(id: string, context: ICustomerDetailsCustomControlContext) {
        ...
        this.dataList = new DataList<ProxyEntities.Customer>({
            columns: [
                {
            ...
            itemDataSource: this.friends,
            selectionMode: SelectionMode.InvokeOnly,
            itemInvoked: this._viewFriendDetails.bind(this)
        });
    }
    ...
    public init(state: ICustomerDetailsCustomControlState): void {
        ...
        if (!this._state.isSelectionMode) {
            ...
            Promise.all(friendLoadPromises).then((): void => {
                this.friends(friends.sort((left: ProxyEntities.Customer, right: ProxyEntities.Customer): number => {
